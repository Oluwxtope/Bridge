...
class Game:
  ...
  def save(self, file_name):
    ...
    KD
    '''
    if self.contract[0].value != "pass":
      file = open(file_name, 'w')
      file.write(self.contract[0].value + '\n')
      file.write(self.contract[0].suit + '\n')
      
      if self.contract[-1] == None:
        file.write(str(self.contract[-1]) + '\n')
      else:
      ...
      file.write(str(len(self.players[0].hand)) + '\n')
      
      for player in self.players:
        file.write(player.name + '\n')
        for card in player.hand:
          file.write(card.value + card.suit + "\n")
      file.close()
##FUNCTIONS TO COMPLETE:
...
def load(file_name):
  ...
  contract = [Bid(lines[0].strip(), lines[1].strip())]
  
  if lines[2].strip() != "None":
    contract.append(Bid(lines[2].strip(), None))
  else:
    contract.append(None)
    
  current_player = lines[3].strip()
  declarer = lines[4].strip()
  num_tricks = int(lines[5].strip())
  ns_vul = lines[6].strip()
  if ns_vul == 'True':
    ns_vul = True
  else:
    ns_vul = False
  ew_vul = lines[7].strip()
  if ew_vul == 'True':
    ew_vul = True
  else:
    ew_vul = False
  player_cards = int(lines[8].strip())
  
  players = []
  for pos in range(9, len(lines), player_cards + 1):
    player = lines[pos].strip()
    hand = []
    for card_pos in range(pos + 1, pos + 1 + player_cards):
      card = lines[card_pos].strip()
      hand.append(Card(card[:-1], card[-1]))
    players.append(Player(player, hand))
  file.close()
  return Game(contract, current_player, declarer, num_tricks, players, ns_vul, ew_vul)
...
P = [Player("North", [Card("2", "S"), Card("K", "D")]), 
     Player("East", [Card("5", "D"), Card("A", "H")]),  
     Player("South", [Card("A", "D"), Card("3", "C")]), 
     Player("West", [Card("K", "D"), Card("8", "S")])]     
